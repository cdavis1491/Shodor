...
void DecideNewPositions()
{
  ...
  // This loop should not be parallelized because it relies on people being
  ...
  for (int personIdx = 0; personIdx < PERSON_COUNT; personIdx++)
  {
    ...
    // This loop should not be parallelized because personCandidateIndices
    ...
    // We won't need more threads than personCandidateCount, which is max 4
    #pragma omp parallel for num_threads(personCandidateCount)
...
void InfectPeople()
{
  ...
  // Loop over each person
  #pragma omp parallel for num_threads(OMP_NUM_THREADS/6)
  for (int personIdx = 0; personIdx < PERSON_COUNT; personIdx++)
  {
    #ifdef DEBUG
      DebugNestedLoop(TimeIdx, personIdx, __FUNCTION__);
...
void PrintFinalCounts()
{
  ...
  #pragma omp parallel for num_threads(OMP_NUM_THREADS) \
                           reduction(+:susceptibleCount) \
                           reduction(+:infectedCount) \
                           reduction(+:recoveredCount)
  for (int personIdx = 0; personIdx < PERSON_COUNT; personIdx++)
  {
